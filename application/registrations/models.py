...
class Registration(Base):
    ...
    def find_course_name(current_id):
        stmt = text("SELECT Course.name, Registration.name, Registration.phonenumber, Registration.email FROM Registration JOIN Course ON Registration.course_name=Course.id LEFT JOIN account ON account.id=Registration.account_id WHERE account.id = :current").params(current = current_id)
