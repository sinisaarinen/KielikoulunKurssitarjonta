...
class Registration(Base):
    ...
    def find_course_name(current_id):
        stmt = text("SELECT Course.name FROM Registration JOIN Course ON Registration.course_name=Course.id LEFT JOIN account ON account.id=Registration.account_id WHERE account.id = :current GROUP BY Course.name").params(current = current_id)
        res = db.engine.execute(stmt)

        response = []
        for row in res:
            response.append({"course_name":row[0]})
